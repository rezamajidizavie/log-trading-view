  ...
  getBars(
    ...
    if (this.debug) {
      ...
      console.log(
        'From:',
        from,
        '(' + new Date(from * 1000).toGMTString() + ')'
      );
        ...
        onHistoryCallback(
          totalKlines.map(kline => {
            if (this.debug) {
              return {
                time: kline[0],
                close: Math.log(parseFloat(kline[4])),
                open: Math.log(parseFloat(kline[1])),
                high: Math.log(parseFloat(kline[2])),
                low: Math.log(parseFloat(kline[3])),
